...
Foam::chemistryGSReductionMethods::DRG<CompType, GThermoType, SThermoType>::DRG
(
    ...
    searchInitSolidSet_(this->coeffsDict_.subDict("initialSolidSet").size())
{
    ...
    {
        if (initGasSet.found(this->chemistry_.Y()[i].member()))
        {
    ...
    if (j<searchInitGasSet_.size())
    {
        FatalErrorInFunction
            ...
            << " species in the initial gas set is not in the mechanism "
            << initGasSet
            << exit(FatalError);
    }
    ...
    {
        if (initSolidSet.found(this->chemistry_.Ys()[i].member()))
        {
    ...
    if (j<searchInitSolidSet_.size())
    {
        FatalErrorInFunction
            ...
            << " species in the initial solid set is not in the mechanism "
            << initSolidSet
            << exit(FatalError);
    }
}
...
template<class CompType, class GThermoType, class SThermoType>
void Foam::chemistryGSReductionMethods::DRG<CompType, GThermoType, SThermoType>::reduceMechanism
(
    const scalar p,
    const scalar T,
    const scalarField& c,
...
)
{
    ...
    forAll(this->chemistry_.solidReactions(), i)
    {
        ...
        // compute the rAB matrix (separate the numerator and
        // denominator)
        DynamicList<scalar> wA(R.lhs().size()+R.rhs().size()+R.glhs().size()+R.grhs().size());
        DynamicList<label> wAID(R.lhs().size()+R.rhs().size()+R.glhs().size()+R.grhs().size());
        ...
        forAll(R.glhs(), g)
        {
            label gs = R.glhs()[g].index;
            label gl = -R.glhs()[g].stoichCoeff;
            bool found(false);
            forAll(wAID, id)
            {
                if (gs==wAID[id])
        ...
        forAll(R.grhs(), g)
        {
            label gs = R.grhs()[g].index;
            scalar gl = R.grhs()[g].stoichCoeff;
            bool found(false);
            forAll(wAID, id)
            {
                if (gs==wAID[id])
    ...
    // (SIS)
    for (label i=0; i<searchInitGasSet_.size(); i++)
    {
        label q = searchInitGasSet_[i];
    ...
    for (label i=0; i<searchInitSolidSet_.size(); i++)
    {
        label q = searchInitSolidSet_[i] + this->nGasSpecie_;
        this->activeSpecies_[q] = true;
        speciesNumber++;
        speciesSolidNumber++;
        Q.push(q);
