...
template<class Type>
void Foam::vtkReader::printFieldStats
(
...
namespace Foam
{
    defineTypeNameAndDebug(vtkReader, 0);

    template<>
    const char*
    NamedEnum<vtkReader::vtkDataType, 8>::names[] =
    {
        "int",
        "unsigned_int",
        "long",
    ...
    const NamedEnum<vtkReader::vtkDataType, 8>
    vtkReader::vtkDataTypeNames;


    template<>
    const char*
    NamedEnum<vtkReader::vtkDataSetType, 3>::names[] =
    {
        "FIELD",
        "SCALARS",
        "VECTORS"
    };
    const NamedEnum<vtkReader::vtkDataSetType, 3>
    vtkReader::vtkDataSetTypeNames;


    template<>
    const char*
    NamedEnum<vtkReader::parseMode, 5>::names[] =
    {
        "NOMODE",
        "UNSTRUCTURED_GRID",
        "POLYDATA",
    ...
    const NamedEnum<vtkReader::parseMode, 5>
    vtkReader::parseModeNames;
}


// * * * * * * * * * * * * * Private Member Functions  * * * * * * * * * * * //

void Foam::vtkReader::warnUnhandledType
(
    Istream& inFile,
    const label type,
...
void Foam::vtkReader::extractCells
(
    Istream& inFile,
...
Foam::wordList Foam::vtkReader::readFieldArray
(
    ISstream& inFile,
    objectRegistry& obj,
...
Foam::objectRegistry& Foam::vtkReader::selectRegistry
(
...
void Foam::vtkReader::read(ISstream& inFile)
{
    ...
    while (inFile.good())
    {
        ...
        else if (tag == "VERTICES")
        {
            ...
            label nVerts(readLabel(inFile));
            label nNumbers(readLabel(inFile));
            if (debug)
            {
                Info<< "Reading " << nVerts << " vertices." << endl;
            }
            labelList faceVerts;
            readBlock(inFile, nNumbers, faceVerts);
        }
