...
#include "error.H"
#include "simpleCGCatalyticChemistry.H"
...
defineTypeNameAndDebug(simpleCGCatalyticChemistry, 0);

addToRunTimeSelectionTable(catalyticChemistryModel, simpleCGCatalyticChemistry, dictionary);

// * * * * * * * * * * * * * * * * Constructors  * * * * * * * * * * * * * * //

// Construct from components
simpleCGCatalyticChemistry::simpleCGCatalyticChemistry
(
    ...
    typeCG_(propsDict_.lookupOrDefault<scalarList>("coarseGrainingFactors", scalarList(1, 1.0)))
{
    ...
    //particleCloud_.checkCG(true);	//Commented
...
void simpleCGCatalyticChemistry::allocateMyArrays() const
{
    double initVal = 0.0;
...
void simpleCGCatalyticChemistry::execute()
{
    forAll(Ygs_, i)
    {
        forAll(Ygs_[i], celli)
    ...
    if(particleCloud_.cg() > 1.)
    {
        ...
        Info << "simpleCatalyticChemistry is using scale from liggghts cg = " << scaleDia_ << endl;
    }
    if (typeCG_.size()>1 || typeCG_[0] > 1)
    {
        Info << "simpleCatalyticChemistry using scale = " << typeCG_ << endl;
    }
...
const volScalarField simpleCGCatalyticChemistry::RR(label i) const
{
    return RR_[i];
}

const volScalarField simpleCGCatalyticChemistry::Qdot() const
{
    return Qdot_;
}

void simpleCGCatalyticChemistry::postFlow()
{
    forAll(ofPartCoverages_, i)
    {
        forAll(ofPartCoverages_[i], index)
