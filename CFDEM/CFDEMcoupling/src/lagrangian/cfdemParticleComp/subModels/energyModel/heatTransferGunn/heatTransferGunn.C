...
void heatTransferGunn::calcEnergyContribution()
{
   // realloc the arrays
    ...
    if(calcPartTempField_)
    {
        ...
        dimensionedScalar aveTemp("aveTemp",dimensionSet(0,0,0,1,0,0,0), gSum(sumTp) / gSum(particleCloud_.averagingM().UsWeightField()));
    ...
    for(int index = 0;index < particleCloud_.numberOfParticles(); ++index)
    {
            ...
            if(cellI >= 0)
            {
                ...
                if(verbose_ && index >=0 && index <2)
                {
                    ...
                    Pout << "kf0 = " << kappaField[cellI] << endl;
                    Pout << "Cp = " << CpField[cellI] << endl;
...
void heatTransferGunn::giveData()
{
    ...
    Info << "net convective particle-fluid heat flux [W] (Eulerian) = " << gSum((QPartFluid_*1.0+QPartFluidCoeff_*tempField_)*QPartFluid_.mesh().V()) << endl;
