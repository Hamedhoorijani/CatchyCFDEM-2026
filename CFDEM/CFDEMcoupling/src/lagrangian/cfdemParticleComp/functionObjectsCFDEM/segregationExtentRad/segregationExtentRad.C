...
namespace functionObjects
{
    ...
    addToRunTimeSelectionTable(functionObject, segregationExtentRad, dictionary);
}
}

// * * * * * * * * * * * * Protected Member Functions  * * * * * * * * * * * //

void Foam::functionObjects::segregationExtentRad::initialise
(
...
void Foam::functionObjects::segregationExtentRad::writeFileHeader
(
    const label i
)
{
...
Foam::functionObjects::segregationExtentRad::segregationExtentRad
(
    const word& name,
    const Time& runTime,
    const dictionary& dict
...
:
    fvMeshFunctionObjectCFDEM(name, runTime, dict),
    logFiles(obr_, name)
{
    read(dict);
    resetName(name);
...
:
    fvMeshFunctionObjectCFDEM(name, obr, dict),
    logFiles(obr_, name)
{
    read(dict);
    resetName(name);
...
bool Foam::functionObjects::segregationExtentRad::read
(
    const dictionary& dict
)
{
    dict.lookup("directionExclude") >> directionExclude_;
    dict.lookup("rmax") >> rmax_;
    dict.lookup("rcut") >> rcut_;
    return true;
}


bool Foam::functionObjects::segregationExtentRad::execute()
{
    return true;
}


bool Foam::functionObjects::segregationExtentRad::write()
{
    ...
    positions_ = particleCloud_.positions();
    ...
    for(int i = 0; i < particleCloud_.numberOfParticles(); i++)
    {
        ...
        for(int j=0;j<3;j++) position_[j] = positions_[i][j];
